...
var ffmpeg = function (/* inputFilepath, settings, callback */) {
	...
	var _ffmpegInfoConfiguration = function (settings) {
		...
		/**
		 * @see {@link https://stackoverflow.com/a/50262835/4650625}
		 */
		utils.exec([ffmpeg.bin,'-formats','2>&1', '-max_muxing_queue_size','1024'], settings, function (error, stdout, stderr) {
	...
	var _videoInfo = function (fileInput, settings) {
		...
		var deferred = when.defer();
		// Make the call to retrieve information about the ffmpeg
		utils.exec([ffmpeg.bin,'-i', utils.addQuotes(fileInput),'2>&1', '-max_muxing_queue_size','1024'], settings, function (error, stdout, stderr) {
